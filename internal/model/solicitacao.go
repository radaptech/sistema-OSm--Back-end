...
// OrdemServico é o corpo de resposta de POST /solicitacoes/:id/abrir-os --
// espelha só o subconjunto de OrdemServico do front que dá pra montar sem
// tocar em os_encerramento/os_custo (fase 2, "Ordem de serviço" no roteiro).
// tecnicoNome/tecnicoArea/empresaTerceirizada* ficam de fora de propósito --
// os três são opcionais no tipo do front (`?`), e a fase 2 já vai precisar
// desses JOINs para o ciclo de vida inteiro; buscar agora seria refazer o
// mesmo trabalho duas vezes. finalizada nasce sempre `false`: uma OS recém
// aberta nunca está encerrada.
...
// MontarOrdemServico traduz a OS recém-criada + a solicitação que a originou
// (já em mãos do service, que a releu para checar o status antes do INSERT)
// pro corpo de resposta. Urgencia/TecnicoId/AfetaProducao vêm à parte porque
// não estão na row nem da OS (RETURNING mínimo, sem JOIN) nem da solicitação
// -- são o que o Gestor decidiu e o que o service computou de
// solicitacao_impacto, respectivamente.
func MontarOrdemServico(os repository.CriarOrdemServicoDeSolicitacaoRow, s repository.ObterSolicitacaoPorIDRow, urgencia string, tecnicoId int64, afetaProducao bool) OrdemServico {
